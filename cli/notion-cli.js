...
async function findPageByTitle(query) {
  ...
  const match =
    res.results.find((item) => getPageTitle(item).toLowerCase().includes(query.toLowerCase())) ||
    res.results[0];
...
const SCAN_PATTERNS = [
  ...
  {
    type: '전화번호',
    re: /\b(01[016789][-.\s]?\d{3,4}[-.\s]?\d{4}|0[2-9]\d?[-.\s]?\d{3,4}[-.\s]?\d{4})\b/g,
  },
  { type: 'IP주소', re: /\b(?:\d{1,3}\.){3}\d{1,3}\b/g },
  {
    type: '접속정보/키',
    re: /\b(api[_-]?key|access[_-]?key|secret|token|password|pwd)\b\s*[:=]\s*\S+/gi,
  },
...
Notion Agent CLI Usage:
  ...
  try {
    if (command === 'search') {
        ...
        children: [
          {
            ...
            toggle: {
              rich_text: [
                { type: 'text', text: { content: toggleTitle }, annotations: { bold: true } },
              ],
          ...
          rich_text: [
            {
              ...
              text: {
                content:
                  '✨ [AI 수정 제안본] 아래 내용은 AI가 새로 다듬은 수정본입니다. 기존 원문은 상단에 보존되어 있으며, 최종 반영하시려면 대화창에 "OK" 또는 "수정 승인"이라고 말씀해주세요.',
              },
            },
          ],
      ...
      console.log(
        `PROPOSED_EDIT_SUCCESS: Appended proposed revision to "${pageQuery}" (${targetPage.id})`,
      );
      ...
      const backupPage = await notion.pages.create({
        ...
        properties: {
          title: {
            title: [{ type: 'text', text: { content: `[백업 원문] ${baseTitle} (${dateStr})` } }],
          },
        },
      ...
      if (deleteFailures.length > 0) {
        console.error(
          `APPROVED_EDIT_PARTIAL: Backed up original to Trash Page (${backupPage.url}), but failed to remove ${deleteFailures.length} block(s) from "${pageQuery}" - the old content may still be visible there alongside the revision. Failures: ${JSON.stringify(deleteFailures)}`,
        );
        process.exitCode = 1;
      } else {
        console.log(
          `APPROVED_EDIT_SUCCESS: Moved original content to Trash Page (${backupPage.url}) and updated "${pageQuery}".`,
        );
      }
      ...
      const trashEntry = await notion.pages.create({
        ...
        children: [
          ...
          },
          {
            ...
            paragraph: {
              rich_text: [
                { type: 'text', text: { content: `${RESTORE_REF_PREFIX}${targetPage.id}]` } },
              ],
            },
          },
        ],
      ...
      console.log(
        `DELETE_SUCCESS: Archived "${title}" and recorded it in Trash Archive (${trashEntry.url}).`,
      );
      ...
      const refBlock = blocks.find(
        (b) => b.type === 'paragraph' && blockText(b).startsWith(RESTORE_REF_PREFIX),
      );

      if (!refBlock) {
        console.log(
          `NO_REF_FOUND: "${trashQuery}" doesn't look like a trash entry created by "delete" (no original-page reference found).`,
        );
        return;
      }
